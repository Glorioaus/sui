...
不要在 skill 中复制解析器行为。需要改变行为时，修改对应的 Python 解析器或配置文件。
...
1. 确认仓库根目录包含 `src/main.py`、`src/merge.py` 和 `requirements.txt`。
2. 确认 Python 虚拟环境已安装依赖。
3. 如果修改过配置，先校验 `config/*.json`。
4. 使用 wrapper 脚本或直接命令运行转换和合并。
5. 检查成功/失败数量，并确认合并输出路径。
